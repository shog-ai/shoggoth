...
async function fetch_dht() {
  // Make a GET request using the fetch API
  await fetch(api_url + "get_dht")
    .then(response => {
    ...
    .then(data => {
      ...
      populate_peers_list(data);
      populate_peers_count(data);
...
async function fetch_pins() {
  // Make a GET request using the fetch API
  await fetch(api_url + "get_pins")
    .then(response => {
      // Check if the response status is OK (200)
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
    ...
    .then(data => {
      ...
      populate_pins_list(data);
      populate_pins_count(data);
