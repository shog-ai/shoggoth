...
//   if (is_inner) {
//     sonic_wildcard_segments_t segments =
...
//         get_files_and_dirs_list(within_resource_path); files_list_t
//         *files_list = UNWRAP(res_files_list);
...
//             strcpy(content, new_res);
//           } else {
//             content = realloc(content, (strlen(content) + strlen(new_res) +
//             1) *
//                                            sizeof(char));
//             strcat(content, new_res);
//           }
...
//           is_file_plain_text(within_resource_path); bool is_plain_text =
//           UNWRAP_BOOL(res_is_plain_text);
...
//             read_file_to_string(within_resource_path); char *file_text =
//             UNWRAP(res_file_text);
...
//             escape_html_tags(escaped_json_string); free(escaped_json_string);

//             content = malloc((strlen(escaped_html_string) + 1) *
//             sizeof(char)); strcpy(content, escaped_html_string);
...
//                 + strlen(download_link) + strlen(resource_size_str) + 1 +
//                 256) *
//                sizeof(char));
...
//   read_file_to_string(head_template_path); char *head_template_string =
//   UNWRAP(res_head_template_string);
...
//                           profile_ctx->config->network.public_host,
//                           shoggoth_id, resource_group_str, resource_name,
//                           profile_ctx->config->network.public_host,
//                           shoggoth_id, resource_group_str, resource_name,
//                           inner_path);
//   if (json_len < 0) {
...
//   snprintf(head_template_data, json_template_size, json_template,
//   shoggoth_id,
//            shoggoth_id, profile_ctx->config->network.public_host,
//            shoggoth_id, resource_group_str, resource_name,
...
//                 (strlen(resource_list) + strlen(new_res) + 1) *
//                 sizeof(char));
...
//           shoggoth_id, resource_group_str, shoggoth_id, main_content,
//           tabs_str);
...
//   "/explorer/profile_og/{shoggoth_id}/{resource_group}",
